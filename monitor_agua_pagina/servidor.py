...
class ServidorMonitorAgua:
        ...
        try:
            ...
            async for mensaje in websocket:
                try:
                    ...
                    if datos.get('action') == 'sending_data':
    ...
    async def manejar_navegador(self, websocket, client_ip):
        """Maneja conexión del navegador web - MODIFICADO"""
        self.conexiones_web.add(websocket)
    
        print(f" Navegador web conectado: {client_ip}")
        ...
        try:
            async for mensaje in websocket:
                try:
                    ...
                    elif data.get('type') == 'request_sessions_history':  # ← NUEVO
                        await self.enviar_historial_sesiones(websocket)
                    elif data.get('type') == 'delete_session':  # ← AGREGAR ESTA LÍNEA
